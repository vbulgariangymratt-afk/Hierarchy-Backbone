...
import { X, Save, Plus, Trash2, Zap, BookOpen, Sparkles, MessageSquare, Loader2 } from 'lucide-react';
import { useStore } from '../../context/StoreContext';
import IslandChat from './IslandChat';
import { callOpenRouter } from '../../services/openrouter';

const IslandDetailModal = ({ habit, onClose }) => {
    const { state, updateHabitContent } = useStore();
    const [notes] = useState(habit.notes || '');
    const [cards, setCards] = useState(habit.cards || []);
    const [activeTab, setActiveTab] = useState('notes'); // 'notes' or 'cards'
    const [loadingIds, setLoadingIds] = useState({});
    const [isGeneratingAll, setIsGeneratingAll] = useState(false);

    const handleSave = () => {
        updateHabitContent(habit.id, notes, cards);
        onClose();
    };
    ...
    const addFlashcardFromChat = (front, back, example, exampleTranslation, audioOverride) => {
        ...
        // Auto-save to global store immediately when adding from chat
        updateHabitContent(habit.id, notes, updatedCards);
    };
    ...
    const generateAIDetail = async (id, phrase) => {
        ...
        try {
            ...
            const area = skill && skill.areaId ? state.areas[skill.areaId] : null;
    ...
    const handleSaveChat = (chatHistory, chatLastUpdated) => {
        updateHabitContent(habit.id, undefined, undefined, chatHistory, chatLastUpdated);
    };
            ...
            <div className="liquid-glass" style={{
                ...
                <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: 'var(--color-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <BookOpen size={20} color="white" />
                        </div>
                        <div>
                            <h2 style={{ fontSize: '22px', fontWeight: '800', letterSpacing: '-0.02em', background: 'linear-gradient(135deg, #fff, #aaa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{habit.name}</h2>
                            <p style={{ fontSize: '13px', opacity: 0.5, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Contextual Knowledge & Flashcards</p>
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: '40px', padding: '0 40px', borderBottom: '1px solid rgba(255,255,255,0.03)', background: 'rgba(255,255,255,0.02)' }}>
                    <button
                        ...
                        style={{
                            ...
                            borderBottom: activeTab === 'notes' ? '2px solid #d4a373' : '2px solid transparent',
                    ...
                    <button
                        ...
                        style={{
                            display: 'flex', alignItems: 'center', gap: '10px', padding: '20px 0',
                            background: 'none', border: 'none',
                            borderBottom: activeTab === 'cards' ? '2px solid #d4a373' : '2px solid transparent',
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
                    {activeTab === 'notes' ? (
                        <IslandChat habit={habit} onSaveCard={addFlashcardFromChat} onSaveChat={handleSaveChat} />
                    ) : (
                                    ...
                                    <div key={card.id} style={{ display: 'flex', flexDirection: 'column', gap: '12px', background: 'rgba(255,255,255,0.02)', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)', position: 'relative' }}>
                                        <div style={{ display: 'flex', gap: '12px' }}>
                                            <div style={{ flex: 1, position: 'relative' }}>
                                                <div style={{ fontSize: '10px', opacity: 0.3, textTransform: 'uppercase', marginBottom: '4px', letterSpacing: '1px' }}>Target Phrase</div>
                                                <input
                                                    value={card.front}
                                            ...
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '10px', opacity: 0.3, textTransform: 'uppercase', marginBottom: '4px', letterSpacing: '1px' }}>Translation</div>
                                                <input value={card.back} onChange={(e) => updateCard(card.id, 'back', e.target.value)} placeholder="e.g. Hello!" style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', padding: '10px', borderRadius: '8px', color: 'rgba(255,255,255,0.7)', fontSize: '15px' }} />
                                            </div>
                                        ...
                                        <div style={{ display: 'flex', gap: '12px' }}>
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <input value={card.example} onChange={(e) => updateCard(card.id, 'example', e.target.value)} placeholder="Target language sentence..." style={{ width: '100%', background: 'none', borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '6px 0', color: '#fff', fontSize: '13px', fontStyle: 'italic', borderTop: 'none', borderLeft: 'none', borderRight: 'none' }} />
                                            </div>
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '10px', opacity: 0.3, textTransform: 'uppercase', marginBottom: '4px', letterSpacing: '1px' }}>Sentence Translation</div>
                                                <input value={card.exampleTranslation} onChange={(e) => updateCard(card.id, 'exampleTranslation', e.target.value)} placeholder="English translation..." style={{ width: '100%', background: 'none', borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '6px 0', color: 'rgba(255,255,255,0.4)', fontSize: '13px', borderTop: 'none', borderLeft: 'none', borderRight: 'none' }} />
                                            </div>
                ...
                <div style={{ padding: '24px 40px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'flex-end', gap: '16px', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(10px)' }}>
                    ...
                    <button onClick={handleSave} style={{
                        ...
                        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 12px 24px rgba(113, 89, 61, 0.4)'; }}
                        onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 16px rgba(113, 89, 61, 0.3)'; }}
                    >
...
export default IslandDetailModal;
